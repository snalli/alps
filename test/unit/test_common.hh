...
struct TestOptions: public Externalizable {
    ...
    size_t kBookSize = 8*1024*1024LLU;
...
ErrorStack TestOptions::load(YAML::Node* node) {
    EXTERNALIZE_LOAD_ELEMENT(node, log_level);
    EXTERNALIZE_LOAD_ELEMENT(node, test_dir);
    EXTERNALIZE_LOAD_ELEMENT(node, book_size);
...
ErrorStack TestOptions::add_command_options(CommandOptionList* cmdlist) {
    ...
    EXTERNALIZE_ADD_COMMAND_OPTION(cmdlist, book_size, "Book size used by the backing file system.");
...
public:
    TestEnvironment(TestOptions& test_options)
        : test_options_(test_options)
    { 
        pegasus_options_.debug_options.log_level = test_options.log_level;
        pegasus_options_.tmpfs_options.booksize = test_options.book_size;
        pegasus_options_.lfs_options.booksize = test_options.book_size;
    }
    ...
    size_t booksize() {
        return test_options_.book_size;
    }
...
private:
    ...
    void cleanup_tmpfs() {
        ...
        system(buf);
        sprintf(buf, "rm -rf %s/*", test_dir().c_str());
        system(buf);
...
protected:
    ...
    std::string test_path(const char* name) {
        std::string test_dir = global_test_env->test_dir();
        test_dir += test_dir.back() != '/' ? "/" : "";
        return test_dir + std::string(name);
    }
    ...
    {
        booksize_ = global_test_env->booksize();
