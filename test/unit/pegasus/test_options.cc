...
ErrorStack ChildOptions::load(YAML::Node* node) {
    EXTERNALIZE_LOAD_ELEMENT(node, foo_bool);
    EXTERNALIZE_LOAD_ELEMENT(node, foo_string);
    EXTERNALIZE_LOAD_ENUM_ELEMENT_OPTIONAL(node, foo_enum, kEnumVal2)
    EXTERNALIZE_LOAD_ELEMENT(node, foo_vector);
    return kRetOk;
}
...
ErrorStack ParentOptions::load(YAML::Node* node) {
