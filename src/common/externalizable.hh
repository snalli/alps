...
struct Externalizable {
   ...
   */
  virtual ErrorStack load(YAML::Node* node) = 0;
   ...
   * Expect errors due to missing-elements, out-of-range values, etc.
   */
  ErrorStack  load_from_string(const std::string& yaml);
  ...
  static ErrorStack get_size_element(YAML::Node* parent, const std::string& tag,
          SIZE* out, bool optional = false, SIZE default_value = static_cast<SIZE>(0)) 
  {
    std::string tmp;
    std::string tmp_default_value = std::to_string(default_value);
    CHECK_ERROR(get_element<std::string>(parent, tag, &tmp, optional, tmp_default_value));
...
/**
 * @def EXTERNALIZE_LOAD_ENUM_ELEMENT(element, attribute, dst)
 * @ingroup EXTERNALIZE
 * @copydoc EXTERNALIZE_LOAD_ELEMENT(element, attribute, dst)
 * @details
...
/**
 * @def EXTERNALIZE_LOAD_SIZE_ELEMENT(element, attribute, dst)
 * @ingroup EXTERNALIZE
 * @copydoc EXTERNALIZE_LOAD_ELEMENT(element, attribute, dst)
 * @details
 * For size, use this one.
 */
#define EXTERNALIZE_LOAD_SIZE_ELEMENT(element, attribute) \
  CHECK_ERROR(get_size_element(element, EX_EXPAND(attribute), & attribute))
/**
...
#define COMPLETE_EXTERNALIZABLE(clazz) \
  ErrorStack load(YAML::Node* node) CXX11_OVERRIDE;\
